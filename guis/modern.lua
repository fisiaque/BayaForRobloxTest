...
function mainapi:CreateGUI()
	...
	settingsversion.Text = 'Baya '..mainapi.Version..' '..(
		isfile('BayaUI/profiles/commit.txt') and readfile('BayaUI/profiles/commit.txt'):sub(1, 6) or ''
	)..' '
...
BayaLogo.Name = 'Logo'
BayaLogo.Size = UDim2.fromOffset(80, 21)
...
BayaLogoV4.Size = UDim2.fromOffset(33, 18)
BayaLogoV4.Position = UDim2.new(1, 1, 0, 1)
...
local BayaLabelCustom = Instance.new('TextLabel')
BayaLabelCustom.Position = UDim2.fromOffset(5, 2)
BayaLabelCustom.BackgroundTransparency = 1
BayaLabelCustom.BorderSizePixel = 0
BayaLabelCustom.Visible = false
BayaLabelCustom.Text = ''
BayaLabelCustom.TextSize = 25
BayaLabelCustom.FontFace = textguifontcustom.Value
BayaLabelCustom.RichText = true
local BayaLabelCustomShadow = BayaLabelCustom:Clone()
BayaLabelCustom:GetPropertyChangedSignal('Position'):Connect(function()
	BayaLabelCustomShadow.Position = UDim2.new(
		BayaLabelCustom.Position.X.Scale,
		BayaLabelCustom.Position.X.Offset + 1,
		0,
		BayaLabelCustom.Position.Y.Offset + 1
	)
end)
BayaLabelCustom:GetPropertyChangedSignal('FontFace'):Connect(function()
	BayaLabelCustomShadow.FontFace = BayaLabelCustom.FontFace
end)
BayaLabelCustom:GetPropertyChangedSignal('Text'):Connect(function()
	BayaLabelCustomShadow.Text = removeTags(BayaLabelCustom.Text)
end)
BayaLabelCustom:GetPropertyChangedSignal('Size'):Connect(function()
	BayaLabelCustomShadow.Size = BayaLabelCustom.Size
end)
BayaLabelCustomShadow.TextColor3 = Color3.new()
BayaLabelCustomShadow.TextTransparency = 0.65
BayaLabelCustomShadow.Parent = textgui.Children
BayaLabelCustom.Parent = textgui.Children
...
function mainapi:UpdateTextGUI(afterload)
	...
	if textgui.Button.Enabled then
		...
		BayaLogo.Position = right and UDim2.new(1 / BayaTextScale.Scale, -113, 0, 6) or UDim2.fromOffset(0, 6)
		BayaLogoShadow.Visible = textguishadow.Enabled
		BayaLabelCustom.Text = textguibox.Value
		BayaLabelCustom.FontFace = textguifontcustom.Value
		BayaLabelCustom.Visible = BayaLabelCustom.Text ~= '' and textguitext.Enabled
		BayaLabelCustomShadow.Visible = BayaLabelCustom.Visible and textguishadow.Enabled
		...
		BayaLabelHolder.Position = UDim2.fromOffset(right and 3 or 0, 11 + (BayaLogo.Visible and BayaLogo.Size.Y.Offset or 0) + (BayaLabelCustom.Visible and 28 or 0) + (textguibackground.Enabled and 3 or 0))
		if BayaLabelCustom.Visible then
			local size = getfontsize(removeTags(BayaLabelCustom.Text), BayaLabelCustom.TextSize, BayaLabelCustom.FontFace)
			BayaLabelCustom.Size = UDim2.fromOffset(size.X, size.Y)
			BayaLabelCustom.Position = UDim2.new(right and 1 / BayaTextScale.Scale or 0, right and -size.X or 0, 0, (BayaLogo.Visible and 32 or 8))
		end
...
function mainapi:UpdateGUI(hue, sat, val, default)
	...
	if textgui.Button.Enabled then
		...
		BayaLogoGradient2.Color = textguigradient.Enabled and textguigradientv4.Enabled and BayaLogoGradient.Color or ColorSequence.new({
			ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)),
			ColorSequenceKeypoint.new(1, Color3.new(1, 1, 1))
		})
		BayaLabelCustom.TextColor3 = textguicolorcustomtoggle.Enabled and Color3.fromHSV(textguicolorcustom.Hue, textguicolorcustom.Sat, textguicolorcustom.Value) or BayaLogoGradient.Color.Keypoints[2].Value
		...
		for i, v in BayaLabels do
			v.Text.TextColor3 = customcolor or (mainapi.GUIColor.Rainbow and Color3.fromHSV(mainapi:Color((hue - ((textguigradient and i + 2 or i) * 0.025)) % 1)) or BayaLogoGradient.Color.Keypoints[2].Value)
			if v.Color then
	...
	for i, v in mainapi.Categories do
		if i == 'Main' then
			v.Object.BayaLogo.V4Logo.ImageColor3 = Color3.fromHSV(hue, sat, val)
