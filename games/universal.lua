...
local baya = shared.baya
local tween = baya.Libraries.tween
local targetinfo = baya.Libraries.targetinfo
local getfontsize = baya.Libraries.getfontsize
local getcustomasset = baya.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('BayaUI/assets/new/blur.png')
...
local function isFriend(plr, recolor)
	if baya.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(baya.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and baya.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(baya.Categories.Targets.ListEnabled, plr.Name) and true
end
...
local function canClick()
	...
	return (not baya.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function notif(...)
	return baya:CreateNotification(...)
end
...
local function serverHop(pointer, filter)
	visited = shared.bayaserverhoplist and shared.bayaserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('Baya', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Baya', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Baya', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Baya', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

baya:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.bayaserverhoplist = '"..table.concat(visited, '/').."'\nshared.bayaserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('BayaUI/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('BayaUI/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('BayaUI/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
baya.Libraries.entity = entitylib
baya.Libraries.whitelist = whitelist
baya.Libraries.prediction = prediction
baya.Libraries.hash = hash
baya.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if baya.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and baya.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(baya.Categories.Friends.Options['Friends color'].Hue, baya.Categories.Friends.Options['Friends color'].Sat, baya.Categories.Friends.Options['Friends color'].Value)
		end
	...
	baya:Clean(baya.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	baya:Clean(baya.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	baya:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	baya:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = baya.Uninject
				baya.Uninject = function()
					notif('Baya', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				baya:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				baya:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('BayaUI/profiles/whitelist.json') and readfile('BayaUI/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				baya:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and baya.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('BayaUI/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.KillBaya then
				baya:Uninject()
				return true
			end
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = baya.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
...
run(function()
	...
	AimAssist = baya.Categories.Combat:CreateModule({
		Name = 'AimAssist',
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not baya.gui.ScaledGui.ClickGui.Visible then
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = baya.gui.AbsoluteSize / 2
...
run(function()
	...
	AutoClicker = baya.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not baya.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = baya.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SilentAim = baya.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
run(function()
	...
	TriggerBot = baya.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
	...
	TargetStrafe = baya.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local flymod, ang, oldent = baya.Modules.Fly or {Enabled = false}
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('BayaUI/assets/new/arrowmodule.png')
	...
	Arrows = baya.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(baya.Categories.Friends.ColorUpdate.Event:Connect(function()
	...
	Chams = baya.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Chams:Clean(baya.Categories.Friends.ColorUpdate.Event:Connect(function()
		...
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(baya.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = baya.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if baya.ThreadFix then
				...
				--movingsound.SoundId = downloadBayaAsset('baya/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadBayaAsset('baya/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = baya.Modules.Fly and baya.Modules.Fly.Enabled or baya.Modules.LongJump and baya.Modules.LongJump.Enabled or baya.Modules.InfiniteFly and baya.Modules.InfiniteFly.Enabled
	...
	NameTags = baya.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(baya.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	Radar = baya:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('BayaUI/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(baya.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	SessionInfo = baya:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.bayasessioninfo = '"..httpService:JSONEncode(baya.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.bayasessioninfo then
					for i, v in httpService:JSONDecode(shared.bayasessioninfo) do
						if baya.Libraries.sessioninfo.Objects[i] and v.Saved then
							baya.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if baya.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
						for i, v in baya.Libraries.sessioninfo.Objects do
	...
	baya:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if baya.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (baya.gui.AbsoluteSize.X / 2)
	...
	baya.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = baya.gui.AbsoluteSize
	...
	Tracers = baya.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(baya.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	AnimationPlayer = baya.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = baya.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = baya.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = baya.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = baya.Categories.Utility:CreateModule({
		Name = 'ChatSpammer',
		Function = function(callback)
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	Disabler = baya.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	baya.Categories.Utility:CreateModule({
		Name = 'Panic',
		Function = function(callback)
			if callback then
				for _, v in baya.Modules do
					if v.Enabled then
						v:Toggle()
					end
				end
...
run(function()
	...
	Rejoin = baya.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = baya.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.bayaserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.bayaserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.bayaserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		if not baya.Loaded then
			repeat task.wait() until baya.Loaded
		end
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				baya.Save = function() end
				if baya.Profile ~= Profile.Value then
					baya.Profile = Profile.Value
					baya:Load(true, Profile.Value)
				end
	...
	StaffDetector = baya.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	baya.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = baya.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = baya.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = baya.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = baya.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = baya.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = baya.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChinaHat = baya.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if baya.ThreadFix then
...
run(function()
	...
	Disguise = baya.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Keystrokes = baya.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = baya.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = baya.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speedmeter = baya.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = baya.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
